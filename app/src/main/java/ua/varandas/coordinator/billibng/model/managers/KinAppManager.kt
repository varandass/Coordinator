...
import ua.varandas.coordinator.billibng.model.*
...
class KinAppManager(private val context: Context, private val developerPayload: String) {
    ...
    fun isBillingSupported(productType: KinAppProductType): Boolean = billingService?.isBillingSupported(KINAPP_API_VERSION, context.packageName, productType.value) == KINAPP_RESPONSE_RESULT_OK
    ...
    fun isBuy(productId: String, productType: KinAppProductType): Boolean {
        ...
        return result == KINAPP_RESPONSE_RESULT_ALREADY_OWNED
    }
        ...
        if (requestCode == KINAPP_REQUEST_CODE) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                if (purchaseData != null) {
                    ...
                    if (purchase.productId.startsWith(TEST_PURCHASE_PREFIX) || (dataSignature != null && SecurityManager.verifyPurchase(developerPayload, purchaseData, dataSignature))) {
    ...
    private fun getResult(responseBundle: Bundle?, responseExtra: String): Int? = responseBundle?.getInt(responseExtra)

    private fun getProduct(productData: String): KinAppProduct {
        val inappProduct = JSONObject(productData)
        return KinAppProduct(product_id = inappProduct.optString("productId"), title = inappProduct.optString("title"), description = inappProduct.optString("description"), price = inappProduct.optString("price"), priceAmountMicros = inappProduct.optLong("price_amount_micros"), priceCurrencyCode = inappProduct.optString("price_currency_code"), type = if (inappProduct.optString("type").equals(SUBS_TYPE, ignoreCase = true)) KinAppProductType.SUBSCRIPTION else KinAppProductType.INAPP)
    }

    private fun getPurchase(purchaseData: String): KinAppPurchase {
        val item = JSONObject(purchaseData)
        return KinAppPurchase(orderId = item.optString("orderId"), productId = item.optString("productId"), purchaseTime = item.optLong("purchaseTime"), purchaseToken = item.optString("purchaseToken"), purchaseState = KinAppPurchaseState.values()[item.optInt("purchaseState", KinAppPurchaseState.CANCELED.value)], packageName = item.optString("purchaseToken"), developerPayload = item.optString("developerPayload"), autoRenewing = item.optBoolean("autoRenewing"))
    }
